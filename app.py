...
def split_reservations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- Convert dates (strip time safely) ---
    df["Arrival"] = pd.to_datetime(df["Arrival"], dayfirst=True, errors="coerce").dt.normalize()
    df["Departure"] = pd.to_datetime(df["Departure"], dayfirst=True, errors="coerce").dt.normalize()
    df["Booking Date"] = pd.to_datetime(df["Booking Date"], dayfirst=True, errors="coerce").dt.normalize()
    ...
    # --- Stay Date → DATEVALUE ---
    df_daily["Date"] = (
        (df_daily["Stay_dates"] - EXCEL_EPOCH)
        .dt.days
        .astype("Int64")
    )
    ...
    df_daily["Booking Date"] = (
        (df_daily["Booking Date"] - EXCEL_EPOCH)
        .dt.days
        .astype("Int64")
    )
    ...
    for col in money_cols:
        if col in df_daily.columns:
            df_daily[col] = (
                pd.to_numeric(df_daily[col], errors="coerce")
                .fillna(0)
                .div(total_nights)
                .round(2)
            )
    ...
    desired_cols = [
        ...
        "Guest Name",
        "Sub Channel",
        "Date",          # Excel DATEVALUE
        "Booking Date",  # Excel DATEVALUE
        "Nights",
    ...
    return df_daily[[c for c in desired_cols if c in df_daily.columns]]
